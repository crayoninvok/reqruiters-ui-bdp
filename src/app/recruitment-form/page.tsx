...
import {
  ...
  Link,
  Users, // Add this
  Lock, // Add this for form closed message
} from "lucide-react";
...
const PublicRecruitmentPage: React.FC = () => {
  ...
  const [openPositions, setOpenPositions] = useState<string[]>([]); // Store open positions from form settings (empty = all open)
  ...
  // Load form options and check form status on component mount
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const formSettingsResponse = await FormSettingsService.getFormSettings();
        const isOpen = formSettingsResponse.formSettings.isFormOpen;
        setIsFormOpen(isOpen);
        
        // Store open positions
        // Jika openPositions ada dan tidak kosong, simpan array tersebut
        // Jika null/undefined atau empty array, berarti semua posisi terbuka
        const positions = formSettingsResponse.formSettings.openPositions;
        if (positions && Array.isArray(positions) && positions.length > 0) {
          // Convert Position enum to string array
          const positionsArray = positions.map(p => String(p));
          setOpenPositions(positionsArray);
        } else {
          setOpenPositions([]); // Empty array means all positions are open
        }

        // Only load options if form is open
        if (isOpen) {
          try {
            const response = await PublicRecruitmentService.getFormOptions();
            setOptions(response.options);
          } catch (optionsError) {
            console.error("Failed to load form options:", optionsError);
          ...
          // Form is closed, set empty options to prevent loading
          setOptions({
            province: [],
            gender: [],
            religion: [],
        ...
        // If error checking form status, assume form is open to allow access
        setIsFormOpen(true);
        try {
          const response = await PublicRecruitmentService.getFormOptions();
          setOptions(response.options);
  ...
  if (isFormOpen === false) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-gray-900 to-slate-900 flex items-center justify-center py-8 px-4">
        <div className="max-w-2xl mx-auto text-center">
          <div className="bg-gradient-to-br from-slate-800/90 via-gray-800/90 to-slate-800/90 backdrop-blur-sm rounded-2xl shadow-2xl border border-slate-600/30 p-8 sm:p-12">
            {/* Icon */}
            <div className="mb-6">
              <div className="mx-auto w-24 h-24 sm:w-28 sm:h-28 bg-red-500/20 rounded-full flex items-center justify-center border border-red-500/30 mb-6">
            ...
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white mb-4">
              Tidak Ada Lowongan Kerja Tersedia
            </h1>

            {/* Description */}
            <div className="space-y-3 mb-8">
              <p className="text-base sm:text-lg text-gray-300 leading-relaxed">
                Maaf, saat ini tidak ada lowongan kerja yang tersedia.
              </p>
              <p className="text-sm sm:text-base text-gray-400">
                Formulir recruitment sedang ditutup. Silakan coba lagi nanti atau hubungi tim HR untuk informasi lebih lanjut.
              </p>
            </div>
            ...
            <button
              ...
              className="px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white rounded-lg sm:rounded-xl font-medium transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl flex items-center justify-center space-x-2 mx-auto"
            >
          ...
          <div className="mt-6 bg-gradient-to-br from-slate-800/50 via-gray-800/50 to-slate-800/50 backdrop-blur-sm rounded-xl border border-slate-600/30 p-4 sm:p-6">
            <div className="flex items-start gap-3">
              ...
              <div className="text-left">
                ...
                <p className="text-xs sm:text-sm text-gray-400 leading-relaxed">
                  Kami akan membuka kembali formulir recruitment ketika ada lowongan kerja yang tersedia. 
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                {options && (
                  <MaritalStatusField
                    ...
                    options={{ maritalStatuses: options.maritalStatuses }}
                ...
                <div>
                  ...
                  >
                    ...
                    {options && options.positions
                      ? options.positions
                          .filter((position) => {
                            // Jika openPositions kosong, tampilkan semua posisi
                            if (openPositions.length === 0) {
                              return true;
                            }
                            // Jika ada openPositions, hanya tampilkan posisi yang ada di list
                            return openPositions.includes(position);
                          })
                          .sort((a, b) =>
                  ...
                  {openPositions && openPositions.length > 0 && (
                    <p className="text-xs text-gray-500 mt-1">
                      Hanya posisi tertentu yang dibuka untuk pendaftaran saat ini
                    </p>
                  )}
                </div>
                <div>
